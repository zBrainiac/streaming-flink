...
### Upload release:
```
scp -i field.pem GoogleDrive/workspace/streaming-flink/target/streaming-flink-0.4.0.0.jar centos@18.192.239.205:/tmp  
...
./bin/kafka-topics --create --bootstrap-server edge2ai-1.dim.local:9092 --replication-factor 1 --partitions 9 --topic kafka_unbalanced
...
cd /opt/cloudera/parcels/FLINK/lib/flink/examples/streaming &&
java -classpath streaming-flink-0.4.0.0.jar producer.KafkaFSICreditCartTRX edge2ai-1.dim.local:9092 333
```
...
./bin/kafka-console-consumer --bootstrap-server edge2ai-1.dim.local:9092 --topic result_fsi-uc4_TrxFxCombined_csv
